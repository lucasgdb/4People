...
const txtLongerRadius = document.querySelector('#longerRadius')
const txtshorterRadius = document.querySelector('#shorterRadius')
const txtMeasure = document.querySelector('#measure')
const txtDecimal = document.querySelector('#decimal')

const calculate = () => {
	if (txtMeasure.value !== '') {
		const area = calculateCircularCrownArea(parseFloat(longerRadius.value), parseFloat(shorterRadius.value))
...
longerRadius.onkeyup = e => {
	if (e.which === 13) shorterRadius.focus()
}

shorterRadius.onkeyup = e => {
