...
try {
	...
	$sql = $database->prepare("SELECT user_name, user_nickname, user_email, user_image, user_level FROM users WHERE user_email=:user_email and user_password=:user_password");
	...
	if ($sql->rowCount()) {
		...
		$_SESSION['logged']['nickname'] = $data[0]['user_nickname'];
